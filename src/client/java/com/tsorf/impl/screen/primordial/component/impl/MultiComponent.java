...
public class MultiComponent extends Component {
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
       ...
       this.setHeight((float) (24 + (extraHeight * animation.getValue())));
        ...
        Tsorf.getInstance().getFonts().getInterMedium().render(context.getMatrices(), selectedModes, getX() + 7, getY() + 12.5f, 6, 0xFFBBBBBB);
        ...
        for (String mode : setting.getModes()) {
            if (getY() + (y * animation.getValue()) > getY() + 15.5)
                Tsorf.getInstance().getFonts().getInterMedium().render(context.getMatrices(), mode, getX() + 7, (float) (getY() + (y * animation.getValue())), 6, setting.getSpecificValue(mode) ? 0xFFB870F2 : 0xFF707070);
            y += open ? 9 : -9;
        }
    }
