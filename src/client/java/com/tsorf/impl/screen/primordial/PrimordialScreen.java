package com.tsorf.impl.screen.primordial;
...
import com.tsorf.api.system.render.animation.Animation;
import com.tsorf.api.system.render.animation.EasingType;
import com.tsorf.api.system.render.font.FontAtlas;
import com.tsorf.common.QuickImports;
import com.tsorf.impl.modules.client.ClickGUI;
import com.tsorf.impl.screen.primordial.component.ModuleComponent;
...
public class PrimordialScreen extends Screen implements QuickImports {
    ...
    public PrimordialScreen() {
        ...
        selectedModule = Tsorf.getInstance().getModuleManager().getModules().stream().filter(m -> m.getCategory() == selectedCategory).toList().getFirst();
    ...
    private void drawBackgroundAndBorders(DrawContext context, MatrixStack matrices, int mouseX, int mouseY) {
        ...
        Identifier logo = Identifier.of("tsorf", "img/logo.png");
        context.drawTexture(renderLayers, logo, (int) (x + 4), (int) (y + 3), 0, 0, 21, 21, 21, 21);

        RenderSystem.disableBlend();
        Tsorf.getInstance().getFonts().getInterMedium().render(context.getMatrices(), "Tsorf", x + 27.5f, y + 4f, 14, 0xFFB870F2);
    ...
    private void drawModuleSelector(MatrixStack matrices) {
        FontAtlas font = Tsorf.getInstance().getFonts().getInterSemiBold();
        FontAtlas medium = Tsorf.getInstance().getFonts().getInterMedium();

        float moduleX = x + 7;
        float currentY = y + 31 + moduleY;

        for (Module module : Tsorf.getInstance().getModuleManager().getModules().stream().filter(m -> m.getCategory() == selectedCategory).toList()) {
    ...
    private void drawCategorySelector(MatrixStack matrices) {
        ...
        for (ModuleCategory category : ModuleCategory.values()) {
            ...
            font.render(matrices, name, currentX, categoryY, 6, selectedCategory
                    ...
                    new Color(197, 197, 197, 255).darker().getRGB());
            Tsorf.getInstance().getFonts().getIcons().renderCenteredString(matrices, category.getIcon(), currentX + nameWidth / 2f - 1.5f, categoryY - 14, 12, selectedCategory
                    == category ? new Color(197, 197, 197, 255).getRGB() :
                    new Color(197, 197, 197, 255).darker().getRGB());
        ...
        if (button == 0) {
            ...
            for (ModuleCategory category : ModuleCategory.values()) {
                ...
                if (mouseX >= boxStartX && mouseX <= boxEndX && mouseY >= boxStartY && mouseY <= boxEndY) {
                    ...
                    moduleY = 0;
                    selectedModule = Tsorf.getInstance()
                            .getModuleManager()
                            .getModules()
                            .stream()
    ...
    @Override
    public void close() {
        Tsorf.getInstance().getModuleManager().getModule(ClickGUI.class).onDisable();
        super.close();
    }
}