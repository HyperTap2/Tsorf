...
public class Notifications extends BaseModule implements QuickImports {
    ...
    public final ModeSetting mode = new ModeSetting(general, "Mode", "Choose style", "Chorus", "Chorus", "Tsorf");
    ...
    private void render2DListener(Render2DEvent event) {
        ...
        for (NotificationManager.Notification notification : notificationManager.getNotifications()) {
            switch (Tsorf.getInstance().getModuleManager().getModule(Notifications.class).mode.getValue()) {
                case "Chorus":
                    notificationManager.setStartingPos(new float[]{mc.getWindow().getScaledWidth() + 500, 0});
                    renderAdjust(matrices, context, notification);
                    break;
                case "Tsorf":
                    renderTsorf(matrices, context, notification);
                    break;
            }
        }
    }
    ...
    private void renderTsorf(MatrixStack matrices, DrawContext context, NotificationManager.Notification notification) {
        FontAtlas poppins = Tsorf.getInstance().getFonts().getPoppins();
        var notificationProgress = notificationManager.getNotificationProgress();
