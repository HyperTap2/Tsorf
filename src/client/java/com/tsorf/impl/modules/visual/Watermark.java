...
public class Watermark extends BaseModule implements QuickImports {
    ...
    private final ModeSetting mode = new ModeSetting(general, "Mode", "Choose watermark style", "Tsorf", "Tsorf", "Gamesense", "Wurst");
    ...
    private void renderTsorf(MatrixStack matrices, DrawContext context) {
        FontAtlas font = Tsorf.getInstance().getFonts().getInterMedium();
        ...
        font.render(matrices, "tsorf.wtf", textX + 13, textY, 7f, Color.WHITE.getRGB());
        setWidth(rectWidth);
    ...
    private void renderGamesense(MatrixStack matrices, DrawContext context) {
        FontAtlas font = Tsorf.getInstance().getFonts().getInterMedium();
        String text = String.format("tsorf | " + mc.player.getNameForScoreboard() + " | %s",
    ...
    private void renderWurst(MatrixStack matrices, DrawContext context) {
        String version = "v" + Tsorf.getInstance().getClientInfo().version() + " MC" + SharedConstants.getGameVersion().getName();
        ...
        Identifier wurst = Identifier.of("tsorf", "img/wurst.png");
