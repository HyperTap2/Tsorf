...
import com.tsorf.impl.screen.hud.HUDEditorScreen;
...
public class TargetHud extends BaseModule implements QuickImports {
    ...
    private final ModeSetting mode = new ModeSetting(general, "Mode", "Choose style", "Tsorf Compact", "Adjust", "Tsorf Compact", "Tsorf Info");
    ...
    private void render2DListener(Render2DEvent event) {
        ...
        PlayerEntity currentEnemy = Tsorf.getInstance().getModuleManager().getModule(Target.class).enemy;
            ...
            if (displayEnemy != null || inHudEditor) {
                switch (mode.getValue()) {
                    case "Adjust":
                        ...
                        break;
                    case "Tsorf Compact":
                        renderTsorfCompact(matrices, context, displayEnemy);
                        break;
                    case "Tsorf Info":
                        renderTsorfInfo(matrices, context, displayEnemy);
                        break;
                }
            }
        }
    ...
    private void renderTsorfCompact(MatrixStack matrices, DrawContext context, PlayerEntity displayEnemy) {
        FontAtlas font = Tsorf.getInstance().getFonts().getInterSemiBold();
        boolean resetToNull = false;
        if (displayEnemy == null) {
            if (mc.currentScreen instanceof HUDEditorScreen) {
    ...
    private void renderTsorfInfo(MatrixStack matrices, DrawContext context, PlayerEntity displayEnemy) {
        FontAtlas font = Tsorf.getInstance().getFonts().getInterSemiBold();
        boolean resetToNull = false;
        if (displayEnemy == null) {
            if (mc.currentScreen instanceof HUDEditorScreen) {
